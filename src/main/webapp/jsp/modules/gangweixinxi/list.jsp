...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">岗位信息管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>岗位信息列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">岗位信息列表</h3>							
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('gongzuoneirong')">工作内容<i id="gongzuoneirongIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('xinzidaiyu')">薪资待遇<i id="xinzidaiyuIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('xuqiurenshu')">需求人数<i id="xuqiurenshuIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function setDataRow(item,number){
			...
			var gongzuoneirongCell  = document.createElement('td');
						 ...
						 gongzuoneirongCell.innerHTML = item.gongzuoneirong;

			row.appendChild(gongzuoneirongCell);
			var xinzidaiyuCell  = document.createElement('td');

						 xinzidaiyuCell.innerHTML = item.xinzidaiyu;

			row.appendChild(xinzidaiyuCell);
			var xuqiurenshuCell  = document.createElement('td');

						 xuqiurenshuCell.innerHTML = item.xuqiurenshu;

			row.appendChild(xuqiurenshuCell);
			...
			var crossBtn0 = document.createElement('button');
            		crossBtn0.innerHTML = "申请岗位"
			crossBtn0.setAttribute("type","button");
			crossBtn0.setAttribute("class","btn btn-primary btn-sm");
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'shenqinggangwei','是','[1]','您已申请过该岗位！')");
			if(crossBtnControl('申请岗位') == true){
		...
		function sh(){
			...
			httpJson("gangweixinxi/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("gangweixinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"gangweixinxi");
